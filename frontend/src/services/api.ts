...
// 通用请求函数
export async function apiRequest(url: string, method = 'GET', data: any = null, isFormData = false, retryCount = 0) {
    ...
    try {
        ...
        try {
            ...
            try {
                ...
                throw new Error(`API响应解析失败，响应数据不是有效的JSON格式: ${responseText.substring(0, 100)}...`);
            }
        }
...
export const api = {
    ...
    admin: {
        ...
        updateUser: (userId: string | number, data: any) => apiRequest(`/admin/user/${userId}`, 'PUT', data),
        ...
        // 用户健康记录
        getUserHealthRecords: (userId: string | number) => apiRequest(`/admin/user/${userId}/health_records`, 'GET'),
        ...
        updateCleaningRules: (rules: any) => {
            ...
            const formattedRules = {
                missing_values: {
                    ...
                    constant_value: rules.missing_values?.constant_value !== undefined 
                ...
                outliers: {
                    ...
                    threshold: rules.outliers?.threshold !== undefined 
                        ? Number(rules.outliers.threshold) 
                        : 3,
        ...
        // 数据可视化
        getVisualizationData: (params: { columns: string[], normalize?: boolean, exclude_outliers?: boolean, compare_by?: string }) => {
        ...
        getFeedbackById: (feedbackId: string | number) => apiRequest(`/admin/feedback/${feedbackId}`, 'GET'),
        updateFeedback: (feedbackId: string | number, data: { status?: string, admin_comment?: string }) => 
