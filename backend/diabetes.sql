...
CREATE TABLE IF NOT EXISTS diabetes_data (
    id INT AUTO_INCREMENT PRIMARY KEY,
    Pregnancies FLOAT NOT NULL COMMENT '怀孕次数',
    Glucose FLOAT NOT NULL COMMENT '口服葡萄糖耐量实验中血浆葡萄糖浓度（2小时）',
    BloodPressure FLOAT NOT NULL COMMENT '舒张压（mmHg)',
    SkinThickness FLOAT NOT NULL COMMENT '肱三头肌皮褶厚度（mm)',
    Insulin FLOAT NOT NULL COMMENT '2 小时血清胰岛素（μU/ml)',
    BMI FLOAT NOT NULL COMMENT '体重指数（体重kg/(身高m)^2)',
    DiabetesPedigreeFunction FLOAT NOT NULL COMMENT '糖尿病谱系功能',
    Age FLOAT NOT NULL COMMENT '年龄（岁）',
